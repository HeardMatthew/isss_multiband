function snr_sd_v4_isss_multi(subj, study, ss)
...
for rr = 1:numruns % For each run... 
    thisrun = [lower(study.prefix), scan.runname num2str(rr)]; 
...
%             spm_write_vol(snr_output, snr);
    ...
    %Return to root
        ...
        %Removes previous .ps files so as not to over-append
            if exist([dir_snr f subj.name '_' thisrun num2str(rr) '_snr.ps'], 'file')
                delete([dir_snr f subj.name '_' thisrun num2str(rr) '_snr.ps']);
            end
...
%             subj2 = regexprep(subjs{ss},'_','\_'); %small fix for figure printing
            ...
            %for j=1:numruns
            %load files
            files_avg = spm_vol([dir_snr_bold f 'AVERAGE' f thisrun '_average.nii']);
            files_sd  = spm_vol([dir_snr_bold f 'SD'      f thisrun '_sd.nii']);
            files_snr = spm_vol([dir_snr_bold f 'SNR'     f thisrun '_snr.nii']);
           ...
           %Title
           ttl = ['SNR\_SD: ' subj.name ' ' thisrun num2str(rr)];
