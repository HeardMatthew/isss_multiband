...
% V8 -- Looking at LIFG/LSTG overlapping in lng > noi across protocols
close all; clc;
tic
...
% JK down to 2 now -- 09/30/19 MJH

%% Paths and parameters
home = pwd; 
dir_rois = 'C:\Users\heard.49\Documents\MATLAB\fmri\isss_multiband_preprocessed_04\rois\lng_noi_overlap_0930219\masks_all'; 
...
dir_docs_betas = fullfile(dir_docs, 'beta_plots_v8'); 
...
files_roi = dir(fullfile(dir_rois, '*.nii')); % down to two!
...
if do_extract    
    ...
    if do_mask
        ...
        filename = fullfile(dir_rois, 'roi_masks_v8.mat'); 
        save(filename, 'mask_master', 'numvox');      
    else
        load(fullfile(dir_rois, 'roi_masks_v8.mat'))
    end
    ...
    if do_betas
        ...
        for rr = these_rois 
            for sc = 1:length(scan)
                ...
                % Dimensions [TR, CON, RUN, SUBJ] and RUN = 1
                
                % for new error bars -- MJH 09/30/19
                betas_total_across_run_subjavg = mean(betas_total_across_run, 1); 
                grandaverage = mean(betas_total_across_run_subjavg, 4); 
                betas_total_across_run_new = betas_total_across_run - betas_total_across_run_subjavg + grandaverage; 
                
                betas_total_across_run_new_sub = mean(betas_total_across_run_new, 4);
                betas_std_dev = nan(numTRs, numCond);
                for cc = these_cond
                    for TR = 1:numTRs
                        foo = reshape(betas_total_across_run_new(TR, cc, 1, :), [1 length(subjects)]);
                ...
                betas_mean_all{sc, rr} = betas_total_across_run_new_sub; 
        ...
        cd(dir_rois)
        save('roi_betas_v8.mat', 'betas_mean_all', 'betas_std_all', 'betas_se_all', 'betas_each_all')
    else
        cd(dir_rois)
        load('roi_betas_v8.mat')
    end
    
    %% Calculate (and export) SPA
    % NOT INCLUDING IN NEW FIGURES -- MJH 09/30/19
...
%                     '1ch Vocoded Noise','Silence','Correct OR','Correct SR','Location','NorthEast');                
                ...
                %% Titles
                name_title = strjoin([roi_title(rr)], ' ');
                name_file = strjoin([scan(sc).name, roi_file(rr)], '_');
                title(name_title, 'FontWeight', 'bold', 'Fontsize', 14)

                %% Save as PNG or SVG or EPS
                if png
                    cd(dir_docs_betas); saveas(gcf,['beta_plot_' name_file], 'png'); cd ..
                end
                
                if svg
%                     cd svg; 
                    cd(dir_docs_betas); saveas(gcf,['beta_plot_' name_file], 'svg'); cd ..
                end
                
                if eps
                    cd(dir_docs_betas); saveas(gcf,['beta_plot_' name_file], 'eps'); cd ..
                end
