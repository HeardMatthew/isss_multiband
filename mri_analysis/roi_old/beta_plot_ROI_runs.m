...
for ROI = 1:numROI % For each ROI...
    ...
    for sc = 1:length(scan) % For each scan type... 
        ...
        betas_scans{sc} = betas_total;
        error_scans{sc} = error_total;
    end % all scans
    ...
    for sc = 1:3 % each scan
        for ss = 1:length(subjects)
            ...
            figure; hold on;
            h1 = plot(x{sc}, betas_scans{sc}(:, 1, 1, ss), 'b'); % noise run 1
            errorbar(x{sc}, betas_scans{sc}(:, 1, 1, ss), error_scans{sc}(:, 1, 1, ss), 'b');

            h2 = plot(x{sc}, betas_scans{sc}(:, 2, 1, ss), 'k'); % silence run 1
            errorbar(x{sc}, betas_scans{sc}(:, 2, 1, ss), error_scans{sc}(:, 2, 1, ss), 'k');
            
            h3 = plot(x{sc}, betas_scans{sc}(:, 3, 1, ss), 'r'); % OR run 1
            errorbar(x{sc}, betas_scans{sc}(:, 3, 1, ss), error_scans{sc}(:, 3, 1, ss), 'r');

            h4 = plot(x{sc}, betas_scans{sc}(:, 4, 1, ss), 'm'); % SR run 1
            errorbar(x{sc}, betas_scans{sc}(:, 4, 1, ss), error_scans{sc}(:, 4, 1, ss), 'm');
            
            h5 = plot(x{sc}, betas_scans{sc}(:, 1, 2, ss), 'b--'); % noise run 2
            errorbar(x{sc}, betas_scans{sc}(:, 1, 2, ss), error_scans{sc}(:, 1, 2, ss), 'b--');

            h6 = plot(x{sc}, betas_scans{sc}(:, 2, 2, ss), 'k--'); % silence run 2
            errorbar(x{sc}, betas_scans{sc}(:, 2, 2, ss), error_scans{sc}(:, 2, 2, ss), 'k--');
            
            h7 = plot(x{sc}, betas_scans{sc}(:, 3, 2, ss), 'r--'); % OR run 2
            errorbar(x{sc}, betas_scans{sc}(:, 3, 2, ss), error_scans{sc}(:, 3, 2, ss), 'r--');

            h8 = plot(x{sc}, betas_scans{sc}(:, 4, 2, ss), 'm--'); % SR run 2
            errorbar(x{sc}, betas_scans{sc}(:, 4, 2, ss), error_scans{sc}(:, 4, 2, ss), 'm--');
            
            xlim([0 11])
            name_title = [scan(sc).name ' ' names{ROI} ' Subject ' num2str(ss)];
            title(name_title, 'FontWeight', 'bold', 'Fontsize', 14)
            name_file = regexp(names{ROI}, '\s+', 'split'); name_file = strjoin([scan(sc).name, name_file, ['s' sn], 'HarvardOxford'], '_');
