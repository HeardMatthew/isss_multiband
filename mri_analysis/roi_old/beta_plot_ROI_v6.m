...
scan(2).name = 'multi_FIR_nowrong';
scan(2).num_TC_per_run = 180;
scan(2).num_TR = 10;
scan(2).num_runs = 2;
scan(2).full = 'Fast Continuous:'; 
...
if do_extract    
    ...
    if do_mask
        ...
        filename = fullfile(dir_rois, 'roi_masks_v6.mat'); 
        save(filename, 'mask_master', 'numvox');      
    else
        load(fullfile(dir_rois, 'roi_masks_v6.mat'))
    end
    
    %% Extract ROI betas
    if do_betas
        betas_mean_all = cell(2, length(rois));
        betas_std_all  = cell(2, length(rois));
        betas_se_all   = cell(2, length(rois));
        % Dimensions [SCAN, ROI]
        ...
        cd(dir_rois)
        save('roi_betas_v6.mat', 'betas_mean_all', 'betas_std_all', 'betas_se_all', 'betas_each_all')
    else
        cd(dir_rois)
        load('roi_betas_v6.mat')
    end
    ...
    if do_spa
        ...
        cd(dir_rois)
        save('roi_SPA_v6.mat', 'SPA_mean_all', 'SPA_std_all', 'SPA_se_all', 'SPA_each_all')
    end
