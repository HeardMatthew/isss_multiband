...
% 04/08/20 -- Cloned for isss_multi, just need to make physio regressors
...
%     end
    ...
    if thissubj.physio
        ...
        cd(dir_setup); make_phys_reg(thissubj, study, 1) % HYBRID
        cd(dir_setup); make_phys_reg(thissubj, study, 2) % ISSS
        cd(dir_setup); make_phys_reg(thissubj, study, 3) % MULTIBAND
        disp('Done!')
    end
