...
function second_level_AUE(subj, study, dd, ss, create, plot)
%% check inputs
if length(subj) < 2
...
%     end
        ...
        if ~contains(thiscon, 'SIL') 
            if design.groupreg
                ...
                load(data)
                if length(subj) == 14
                    if strcmp(thiscon, 'OR_SR')
                        matlabbatch{1}.spm.stats.factorial_design.cov.c = group_level_OR_SR; 
                    elseif strcmp(thiscon, 'OR_NOI')
                        matlabbatch{1}.spm.stats.factorial_design.cov.c = group_level_OR; 
                    elseif strcmp(thiscon, 'SR_NOI')
                        matlabbatch{1}.spm.stats.factorial_design.cov.c = group_level_SR; 
                    else % LNG contrasts
                        matlabbatch{1}.spm.stats.factorial_design.cov.c = group_level_acc; 
                    end
                    
                else % hard coding how to drop first subject!
                    if strcmp(thiscon, 'OR_SR')
