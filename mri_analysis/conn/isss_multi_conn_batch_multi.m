...
%   the regressor code. 
% 04/10/20 -- Ran for isss and multi. Loading the hybrid CONN_x files lets
%   me re-use the ROI/anatomical info!
...
clearvars; clc; isss_multi_params
...
ss = 3; % MULTIBAND
conn_name = 'conn_multi_physio_v2'; 
scan = study.scan(ss);

if load_batch
    load conn_hybrid_physio.mat 
    % has all the anatomical files and segmented compartments pre-loaded!
    ...
    for nsub = 1:length(subj)
        ...
        %% Anatomical files (ONLY RUN FOR HYBRID!)
...
%             ['[1 file] x [size ' num2str(V.dim) ' ]']; 
        ...
        for nses = 1:nsessions
            target = fullfile(dir_func_GLM, ... 
                [study.prefix scan.runname num2str(nses) '*.nii']); 
            files_func = dir(target); 
            files_func = fullfile(dir_func_GLM, {files_func(:).name})';
...
if create_reg
    ...
    for ii = 1:length(subj) % PICK UP HERE
        ...
        for rr = blocks
            ...
            fname = fullfile(dir_func_GLM, ...
                ['dp_' study.prefix scan.runname num2str(rr) '_' regnum '.txt']); 
            if exist(fname, 'file')
            ...
            newname = fullfile(dir_reg, ...
                ['dp_trimmed_pc2_' study.prefix scan.runname num2str(rr) '_' regnum '.txt']); 
