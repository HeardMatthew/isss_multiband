...
function coord_sphere(subj, study, dd, ss)
%% Check input
if ~isstruct(subj) || length(subj) ~= 1
    error('Input (subj, study, dd, ss) where subj is a SINGLE struct')
end

if ~isstruct(study)
    error('Input (subj, study, dd, ss) where study is struct')
end

if ~isnumeric(dd)
    error('Input (subj, study, dd, ss) where dd specifies which design')
end
...
%% Save file
filename = fullfile(dir_data_MVPA, [subj.name '_' scanname '_' design.name '_spheres_radius' num2str(study.mvpa.radius) '.mat']); 
