...
function get_wb_accuracy(subj, study, dd, classifier, scans)
%% check inputs
if length(subj) < 2
...
voxels_in_mask = xyz(:, combine_mask(:)); 

%% And then just look at significant voxels (this is whole brain...)
% fname = fullfile(dir_designs{1}, 'results.nii');
% V = spm_vol(fname); 
% y = spm_read_vols(V); y = logical(y); 
% combine_mask = combine_mask & y; 
...
for nscan = 1:numscans
    ...
    for nsubj = 1:numsubjs
        ...
        ACC(nsubj) = mean(data(combine_mask)); 
    end
...
fname = fullfile(dir_mvpa, ['ACC_' classifier '_' scannames '_wholebrain_091420.mat']); 
