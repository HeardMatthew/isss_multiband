...
function searchlight_LNG_NOI(subj, study, dd, ss)
%% Check input
...
filename = fullfile(dir_data_MVPA, ...
    [subj.name '_' scanname '_' design.name '_betas.mat']);
load(filename)

filename = fullfile(dir_data_MVPA, ...
    [subj.name '_' scanname '_' design.name '_spheres_radius' num2str(radius) '.mat']);  
load(filename);  

%% More parameters and preallocation
numvox = length(sphere_XYZ_indices_cell);  %#ok<USENS>

crossval_acc_mat_folds = zeros(subj.runs(ss), numvox);
...
V.fname = fullfile(dir_data_MVPA, ... 
    [subj.name '_' scanname '_' design.name '_beta_GNB_LNG_NOI_rad' num2str(radius) '.nii']);  
spm_write_vol(V, acc);
