...
try
    for j = 1:p.events
        ...
        RTBox('Clear'); 
        WaitTill(jitterKey(j) + presentStart(j)); 
        PsychPortAudio('Start', pahandle);
        WaitTill(jitterKey(j) + presentStart(j) + stimDuration(eventKey(j)));
        PsychPortAudio('Stop', pahandle, 1);
        presentEnd(j) = GetSecs();

        [respTimeKey{j}, responseKey{j}] = RTBox(p.respWindow); 
        if ~strcmp(responseKey{j}, '')
            [~, RTEnd(j)] = WaitTill(presentEnd(j) + p.respWindow); 
        end

        [~, epiEnd(j)] = WaitTill(presentStart(j) + p.stimTime + p.epiTime);    
    end
